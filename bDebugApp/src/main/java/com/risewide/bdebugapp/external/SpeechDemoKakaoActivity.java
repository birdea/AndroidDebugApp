...
import com.risewide.bdebugapp.external.kakao.KakaoApiConfig;
 ...
 */
	...
	public class SpeechDemoKakaoActivity extends Activity implements View.OnClickListener, SpeechRecognizeListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_demo_kakao);
	...
	public void onClick(View v) {
		...
		if (id == R.id.speechbutton) {
			if(PermissionUtils.checkAudioRecordPermission(this)) {
				SpeechRecognizerClient.Builder builder = new SpeechRecognizerClient.Builder().
						setApiKey(KakaoApiConfig.APIKEY).
						setServiceType(serviceType);

				if (serviceType.equals(SpeechRecognizerClient.SERVICE_TYPE_WORD)) {
					String wordList = words_edit.getText().toString();
		...
		// 음성인식 기본 UI 버튼 listener
			...
			i.putExtra(SpeechRecognizerActivity.EXTRA_KEY_API_KEY, KakaoApiConfig.APIKEY);
			if (serviceType.equals(SpeechRecognizerClient.SERVICE_TYPE_WORD)) {
				String wordList = words_edit.getText().toString();
	...
	public void onPartialResult(final String text) {
		...
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				words_edit.setText(String.valueOf(text));
			}
		});
	}
	...
	public void onResults(Bundle results) {
		...
		final String lastText = (texts.size()>0)?texts.get(0):"";
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				words_edit.setText(String.valueOf(lastText));
